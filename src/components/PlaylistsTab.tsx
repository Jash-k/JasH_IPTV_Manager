...
import {
  ...
  Download, ExternalLink, List, Star, Server, Edit3,
} from 'lucide-react';
...
export default function PlaylistsTab({ onEditPlaylist }: Props) {
  const {
    playlists, groups, channels, serverUrl,
    createPlaylist, updatePlaylist, deletePlaylist,
    getPlaylistM3U, exportDB,
  } = useStore();
  ...
  const toggleGroupSel = (groupName: string, list: 'include' | 'exclude') => {
    ...
    setForm(f => {
      const updated = f[key].includes(groupName)
        ? f[key].filter(g => g !== groupName)
        : [...f[key], groupName];
  ...
  const handleSubmit = () => {
    ...
    if (editId) {
      ...
      const pl = createPlaylist(form.name, form.includeGroups, form.tamilOnly);
      toast.success('✅ Playlist created!');
      // Auto-open editor for the new playlist
      onEditPlaylist(pl.id);
    }
  ...
  const getPlaylistChannelCount = (pl: {
    ...
    const pinned  = new Set(pl.pinnedChannels  || []);
    const blocked = new Set(pl.blockedChannels || []);
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-blue-400 text-xs leading-relaxed">
              Each playlist URL (<code className="font-mono bg-blue-900/30 px-1 rounded">/api/playlist/&#123;id&#125;.m3u</code>) is served live by
              the backend. Add channels in the <strong>Playlist Editor</strong> — use the ✏️ Edit button on each playlist card.
              {tamilChannelCount > 0 && ` You have ${tamilChannelCount} Tamil channels available.`}
            </p>
          </div>
        </div>
      </div>
      ...
      {showAdd && (
        <div className="bg-gray-800 border border-gray-700 rounded-xl p-5 space-y-4 shadow-xl">
          ...
          >
            ...
            <div>
              ...
              <p className="text-gray-500 text-xs">
                {form.tamilOnly
                  ? `Will include only ${tamilChannelCount} Tamil channels (Sun TV, Vijay, Zee Tamil, Polimer…)`
                  ...
                  {groups.map(g => (
                    <button key={g.id} onClick={() => toggleGroupSel(g.name, 'exclude')}
                      className={`px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${
                        form.excludeGroups.includes(g.name)
          ...
          <div className="flex gap-2 justify-end pt-1">
            <button onClick={() => setShowAdd(false)} className="px-4 py-2 text-gray-400 hover:text-white text-sm">
              Cancel
            </button>
            <button onClick={handleSubmit}
              className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium">
              <Save className="w-4 h-4" />
              {editId ? 'Update' : 'Create & Open Editor'}
            </button>
          </div>
        </div>
      )}
      ...
      {playlists.length === 0 ? (
        ...
        <div className="space-y-3">
          {playlists.map(pl => {
            ...
            const pinnedCount       = (pl.pinnedChannels  || []).length;
            const blockedCount      = (pl.blockedChannels || []).length;
            const tamilInPlaylist   = channels.filter(ch => {
              const pinned  = new Set(pl.pinnedChannels  || []);
              const blocked = new Set(pl.blockedChannels || []);
              if (blocked.has(ch.id)) return false;
              if (pinned.has(ch.id))  return ch.isTamil;
              if (!ch.isActive) return false;
              if (pl.tamilOnly && !ch.isTamil) return false;
              if (pl.includeGroups.length && !pl.includeGroups.includes(ch.group)) return false;
              if (pl.excludeGroups.includes(ch.group)) return false;
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mb-3 flex-wrap text-xs">
                      <span className="flex items-center gap-1">
                        ...
                        <span className="text-green-400 font-semibold">{channelCount}</span>
                        <span className="text-gray-500">channels</span>
                      </span>
                      {tamilInPlaylist > 0 && (
                        <span className="flex items-center gap-1">
                          <Star className="w-3 h-3 text-orange-400 fill-orange-400" />
                          <span className="text-orange-400 font-semibold">{tamilInPlaylist}</span>
                          <span className="text-gray-500">Tamil</span>
                        </span>
                      )}
                      {pinnedCount > 0 && (
                    ...
                    {/* ── EDIT CHANNELS BUTTON — prominent ── */}
                    <button
                      onClick={() => onEditPlaylist(pl.id)}
                      className="flex items-center gap-2 bg-pink-600 hover:bg-pink-700 text-white px-4 py-2 rounded-lg text-sm font-semibold transition-colors mb-3 shadow-lg shadow-pink-900/30"
                    >
                      <Edit3 className="w-4 h-4" />
                      Edit Channels in Playlist
                      <span className="text-xs bg-pink-500 text-white px-1.5 py-0.5 rounded font-bold">{channelCount}</span>
                    </button>
                    ...
                    <div className="p-3 bg-gray-900 rounded-xl border border-gray-700 hover:border-blue-700 transition-colors">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={() => copyUrl(serverPlaylistUrl, pl.id)}
                          className="shrink-0 text-gray-400 hover:text-white p-1.5 hover:bg-gray-700 rounded-lg transition-colors"
                        >
                  ...
                  <div className="flex flex-col gap-1 shrink-0">
                    ...
                    <button
                      onClick={() => {
                        setForm({
                          name: pl.name,
                          includeGroups: pl.includeGroups,
                          excludeGroups: pl.excludeGroups,
                          tamilOnly: pl.tamilOnly || false,
                        });
                        setEditId(pl.id);
                        setShowAdd(true);
                      }}
