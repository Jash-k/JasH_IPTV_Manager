...
const QUALITY_PATTERNS: { pattern: RegExp; label: string }[] = [
  { pattern: /\b(2160p|4k|uhd)\b/i,         label: '4K'   },
  { pattern: /\b(1080p|fhd|fullhd)\b/i,     label: '1080p'},
  { pattern: /\b(720p|hd)\b/i,              label: '720p' },
  { pattern: /\b(480p|sd)\b/i,              label: '480p' },
...
// ── Title cleaner ─────────────────────────────────────────────────────────────
// Removes quality tags and trailing/leading junk from a raw title string
// e.g. "Velaikkaran (1987) (720p)" → title:"Velaikkaran", year:1987, quality:"720p"
function parseMovieTitle(raw: string): { title: string; year?: number; quality: string } {
  let s = (raw || '').trim();

  // 1. Extract quality from brackets anywhere in string
  const quality = detectQuality(s);

  // 2. Remove quality brackets: "(720p)", "[1080p]", "(HD)", etc.
  s = s.replace(/[\(\[]\s*(4k|uhd|2160p|fhd|1080p|720p|480p|360p|hd|sd)\s*[\)\]]/gi, '');

  // 3. Extract year — look for 4-digit year between 1900–2030
  let year: number | undefined;
  const yearMatch = s.match(/[\(\[]((?:19|20)\d{2})[\)\]]/);
  if (yearMatch) {
    year = parseInt(yearMatch[1]);
    s = s.replace(yearMatch[0], ''); // remove the year bracket
  } else {
    // trailing year
    const trailYear = s.match(/\s+((?:19|20)\d{2})\s*$/);
    if (trailYear) {
      year = parseInt(trailYear[1]);
      s = s.replace(trailYear[0], '');
    }
  }

  // 4. Clean up leftover punctuation / double spaces
...
// ── M3U attribute extractor ───────────────────────────────────────────────────
function extractAttr(line: string, attr: string): string {
  const m = line.match(new RegExp(`${attr}="([^"]*)"`, 'i'));
  return m ? m[1].trim() : '';
}

// ── FIXED: last unquoted comma extractor ─────────────────────────────────────
function extractCommaName(extinf: string): string {
  ...
  return fb !== -1 ? extinf.substring(fb + 1).trim() : 'Unknown';
}

// ── Main M3U / JSON parser ─────────────────────────────────────────────────────
type PartialMovie = Omit<MovieStream,
  'tmdbId'|'imdbId'|'poster'|'backdrop'|'overview'|'rating'|'genres'|'releaseDate'|'runtime'>;

function parseMovieContent(content: string, sourceId: string, sourceName: string): PartialMovie[] {
  ...
  if (trimmed.startsWith('[') || trimmed.startsWith('{')) {
    try {
      ...
      arr.forEach((item, i) => {
        const url = String(item.link || item.url || item.stream || item.src || item.streamUrl || item.playbackUrl || '');
        if (!url || !url.startsWith('http')) return;

        const rawName = String(item.name || item.title || item.movie || item.channel || `Movie ${i + 1}`);
        const { title, year, quality } = parseMovieTitle(rawName);
        ...
        streams.push({
          ...
          year,
          url,
          quality     : detectQuality(rawName + ' ' + url) || quality,
          logo        : String(item.logo || item.icon || item.image || item.poster || item.thumbnail || ''),
          group       : sourceName,  // ← source name becomes the group
          sourceId,
          enabled     : true,
          source      : sourceName,
  ...
  let pendingExtinf = '';
  let pendingLicenseType = '', pendingLicenseKey = '';
  let pendingUserAgent = '', pendingCookie = '';
  let pendingLogo = '';

  const reset = () => {
    pendingExtinf = ''; pendingLicenseType = ''; pendingLicenseKey = '';
    pendingUserAgent = ''; pendingCookie = ''; pendingLogo = '';
  };
  ...
  for (const line of lines) {
    ...
    if (line.startsWith('#KODIPROP:inputstream.adaptive.license_type=')) {
      pendingLicenseType = line.split('=').slice(1).join('=').trim(); continue;
    }
    if (line.startsWith('#KODIPROP:inputstream.adaptive.license_key=')) {
      pendingLicenseKey = line.split('=').slice(1).join('=').trim(); continue;
    }
    if (line.startsWith('#EXTVLCOPT:http-user-agent=')) {
      pendingUserAgent = line.replace('#EXTVLCOPT:http-user-agent=', '').trim(); continue;
    }
    ...
    if (!line.startsWith('#') && line.startsWith('http')) {
      ...
      let rawTitle = '';
      if (pendingExtinf) {
        // Priority 1: tvg-name attribute — most reliable
        const tvgName = extractAttr(pendingExtinf, 'tvg-name');
        // Priority 2: last unquoted comma name
        const commaName = extractCommaName(pendingExtinf);
        // Use tvg-name if it looks cleaner (shorter or doesn't have junk)
        rawTitle = tvgName || commaName || 'Unknown';
      } else {
        rawTitle = line.split('/').pop()?.replace(/\.(m3u8?|mpd|ts|mp4).*$/i, '') || 'Unknown';
      }

      const { title, year, quality } = parseMovieTitle(rawTitle);
      ...
      streams.push({
        ...
        quality,
        logo        : pendingLogo,
        group       : sourceName,  // ← source name = group
        sourceId,
        enabled     : true,
        source      : sourceName,
...
// ── Normalize title for dedup/combine ─────────────────────────────────────────
function normalizeTitle(title: string): string {
  return (title || '')
    .toLowerCase()
    .replace(/\b(4k|uhd|fhd|hd|sd|1080p|720p|480p|2160p|bluray|blu-ray|webrip|web-dl|dvdrip)\b/gi, '')
    .replace(/[^\w\s]/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
}

// ── Remove duplicates (same title+year+url) ───────────────────────────────────
function removeDuplicates(streams: MovieStream[]): MovieStream[] {
  const seen = new Map<string, MovieStream>();
  for (const s of streams) {
    const key = normalizeTitle(s.title) + '|' + (s.year || '') + '|' + (s.url || '');
    if (!seen.has(key)) seen.set(key, s);
  }
  return [...seen.values()];
}

// ── Combine by quality (same title+year, different quality) ───────────────────
...
async function fetchTMDB(title: string, year: number | undefined, apiKey: string): Promise<Partial<MovieStream>> {
  ...
  if (!res.ok) {
    if (res.status === 401) throw new Error('Invalid TMDB API key (401 Unauthorized)');
    return {};
  }
  ...
  let genres: string[] = [], runtime: number | undefined, imdbId: string | undefined;
  try {
    const det = await fetch(
      `https://api.themoviedb.org/3/movie/${movie.id}?api_key=${apiKey.trim()}&append_to_response=external_ids`,
...
const MovieAddonTab: React.FC = () => {
    ...
    const iv = setInterval(async () => {
      ...
      for (const src of due) {
        try {
          ...
          const parsed = parseMovieContent(content, src.id, src.name).map((s, i) => ({
            ...s,
            id: `mov_${src.id}_${i}_${Date.now()}`,
            tmdbId: undefined as undefined, imdbId: undefined as undefined,
            poster: undefined as undefined, backdrop: undefined as undefined,
            overview: undefined as undefined, rating: undefined as undefined,
            genres: undefined as undefined, releaseDate: undefined as undefined,
          ...
          const upd = { ...src, streamCount: parsed.length, status: 'active' as const,
            lastUpdated: now, nextAutoRefresh: now + (src.autoRefreshInterval ?? 0) * 60_000 };
          const us = sources.map(s => s.id === src.id ? upd : s);
  ...
  // ── Process content into MovieStream[] ────────────────────────────────────
  const processContent = useCallback((content: string, srcId: string, srcName: string, existing: MovieStream[]): MovieStream[] => {
    const parsed = parseMovieContent(content, srcId, srcName).map((s, i) => ({
      ...s,
    ...
    let finalStreams = [...existing.filter(s => s.sourceId !== srcId), ...parsed];
  ...
  const addSource = useCallback(async () => {
    ...
    const name  = newSourceName.trim() || newSourceUrl.split('/').pop()?.replace(/\.(m3u8?|json)$/i, '') || 'Movie Source';
  ...
  const fetchAllMetadata = useCallback(async (streamList: MovieStream[], apiKey: string) => {
    ...
    const groups  = combineByQuality(streamList.filter(s => s.enabled));
    const toFetch = groups.filter(g => !g.metadata?.tmdbId && g.title !== 'Unknown');
    ...
    for (let i = 0; i < toFetch.length; i++) {
      ...
      const group = toFetch[i];
      setFetchingMeta([group.title]);
      try {
        ...
        if (msg.includes('401') || msg.includes('Invalid TMDB')) {
          setTmdbError(`❌ TMDB Error: ${msg}`);
          break;
        }
      }
      setFetchProgress(Math.round((i + 1) / toFetch.length * 100));
      saveLS(MOVIE_STREAMS_KEY, [...updated]);
      setStreams([...updated]);
    ...
    setFetchingMeta([]);
    setFetchProgress(0);
    if (done > 0) notify(`✅ Fetched metadata for ${done} movies`, 'success');
  }, [notify]);

  // ── Filtered movies ────────────────────────────────────────────────────────
  const enabledStreams  = streams.filter(s => s.enabled);
  const combinedMovies  = combineByQuality(
    filterSource ? enabledStreams.filter(s => s.sourceId === filterSource) : enabledStreams
  );
  ...
  const filteredMovies = combinedMovies.filter(m => {
    ...
    if (filterRating  && (m.metadata?.rating || 0) < parseFloat(filterRating))      return false;
    return true;
  });
  ...
  const exportM3U = () => {
    ...
    streams.filter(s => s.enabled).forEach(s => {
      ...
      lines.push(`#EXTINF:-1 tvg-name="${title}" tvg-logo="${s.logo || ''}" group-title="${s.group || 'Movies'}",${title}`);
  ...
  const notifColor = { success: 'bg-green-600', error: 'bg-red-600', info: 'bg-blue-600', warn: 'bg-amber-600' };
  ...
  return (
    <div className="h-full flex flex-col bg-gray-950">
      ...
      {/* Notification */}
      {notification && (
        <div className={`fixed top-4 right-4 z-50 px-4 py-3 rounded-xl text-white text-sm shadow-2xl flex items-center gap-2 max-w-sm ${notifColor[notification.type]}`}>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-gray-400 text-xs mt-0.5">
              {enabledStreams.length} streams · {combineByQuality(enabledStreams).length} unique movies · {sources.length} sources
              {streams.filter(s => s.tmdbId).length > 0 && ` · ${streams.filter(s => s.tmdbId).length} with TMDB`}
            </p>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            <button onClick={exportM3U} className="bg-blue-600 hover:bg-blue-700 text-white text-xs px-3 py-2 rounded-lg transition-colors flex items-center gap-1">
              ⬇️ M3U
            </button>
            <button onClick={handleMovieSync}
              disabled={syncing || !backendOnline || !enabledStreams.length}
              className={`flex items-center gap-1.5 text-xs px-3 py-2 rounded-lg transition-colors ${
                syncing ? 'bg-violet-800 text-violet-300 animate-pulse' :
                backendOnline ? 'bg-violet-600 hover:bg-violet-700 text-white' : 'bg-gray-700 text-gray-500 cursor-not-allowed'
              }`}>
          ...
          <div className="p-6 space-y-4">
            <div className="bg-blue-900/20 border border-blue-700/30 rounded-xl p-3">
              <p className="text-blue-300 text-xs">
                <strong>📂 Source = Group</strong> — Each source becomes its own group in Stremio.
                Name your sources meaningfully: "Tamil Movies", "Hollywood", "Dubbed", "New Releases", etc.
              </p>
            </div>
            ...
            {showAddSource && (
              <div className="bg-gray-900 border border-yellow-500/30 rounded-xl p-4 space-y-3">
                ...
                <div>
                  <label className="text-gray-400 text-xs block mb-1">Source Name (becomes group name in Stremio) *</label>
                  <input type="text" placeholder="e.g. Tamil Movies, Hollywood, New Releases…"
                ...
                <input type="text" placeholder={newSourceType === 'json' ? 'https://example.com/movies.json' : 'https://raw.github.com/.../movies.m3u'}
                      ...
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-1 flex-wrap">
                            ...
                            {src.error && <span className="text-xs text-red-400 truncate max-w-xs">{src.error}</span>}
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        {src.url && (
                          <button onClick={() => refreshSource(src)}
                            className="bg-blue-600/20 hover:bg-blue-600/40 text-blue-400 px-2 py-1.5 rounded text-xs transition-colors">
                            🔄 Refresh
                          </button>
                        )}
                        <button onClick={() => deleteSource(src.id)}
                          className="bg-red-500/20 hover:bg-red-500/40 text-red-400 px-2 py-1.5 rounded text-xs transition-colors">
                          🗑️
                        </button>
                      </div>
                    </div>
                      ...
                      <div className="mt-3 flex items-center gap-2">
                        ...
                        <select value={src.autoRefreshInterval || 0}
                          onChange={e => {
                            ...
                            const upd = { ...src, autoRefreshInterval: val, nextAutoRefresh: val > 0 ? Date.now() + val * 60_000 : undefined };
          ...
          <div className="p-6 space-y-4">
            ...
            {tmdbError && (
              <div className="bg-red-900/30 border border-red-700/40 rounded-xl p-3 flex items-start gap-2">
                <span className="text-red-400 text-sm flex-1">{tmdbError}</span>
                <button onClick={() => setTmdbError(null)} className="text-gray-500 hover:text-white text-xs">✕</button>
              </div>
            )}
            ...
            <div className="bg-gray-900 border border-white/10 rounded-xl p-4 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => { setSearchQuery(''); setFilterQuality(''); setFilterYear(''); setFilterRating(''); setFilterGenre(''); setFilterSource(''); }}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2">
                <input type="text" placeholder="🔍 Search title…" value={searchQuery}
                ...
                <select value={filterYear} onChange={e => setFilterYear(e.target.value)}
                  ...
                  {years.slice(0, 40).map(y => <option key={y} value={String(y)}>{y}</option>)}
                </select>
                ...
                {settings.tmdbApiKey ? (
                  <>
                    <button onClick={() => fetchAllMetadata(streams, settings.tmdbApiKey)}
                      ...
                      className="bg-blue-600 hover:bg-blue-700 text-white text-xs px-3 py-1.5 rounded-lg transition-colors disabled:opacity-50 flex items-center gap-1.5">
                      {fetchingMeta.length > 0 ? <>⏳ {fetchingMeta[0]}…</> : <>🎭 Fetch TMDB Metadata</>}
                    </button>
                    {fetchingMeta.length > 0 && (
                      <>
                        <div className="bg-gray-800 rounded-full h-1.5 flex-1 overflow-hidden min-w-16">
                          <div className="bg-blue-500 h-full transition-all duration-300" style={{ width: `${fetchProgress}%` }} />
                        </div>
                        <span className="text-gray-400 text-xs">{fetchProgress}%</span>
                        <button onClick={() => { abortRef.current?.abort(); setFetchingMeta([]); }}
                          className="text-red-400 text-xs hover:text-red-300 transition-colors">Stop</button>
                      </>
                    )}
                  </>
                ) : (
                  <p className="text-amber-400/70 text-xs">
                    ⚠️ Add TMDB API key in Settings to fetch movie posters & ratings
                    <button onClick={() => setActiveSection('settings')} className="text-yellow-400 underline ml-1">→ Settings</button>
                  </p>
                )}
              </div>
            </div>
            ...
            {filteredMovies.length === 0 ? (
              <div className="text-center py-12 text-gray-500">
                ...
                <p className="text-xs mt-1">Add sources or adjust filters</p>
              </div>
            ) : (
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
                {filteredMovies.slice(0, 300).map(movie => {
                  ...
                  const qualities2 = [...new Set(movie.streams.map(s => s.quality).filter(Boolean))];
                  return (
                    <div key={movie.title + (movie.year || '')}
                      ...
                      <div className="relative aspect-[2/3] bg-gray-800">
                        ...
                        {movie.streams.length > 1 && (
                          <div className="absolute bottom-1 right-1 bg-black/80 text-white text-xs px-1.5 rounded">
                        ...
                        {movie.streams.some(s => s.licenseType) && (
                          <div className="absolute bottom-1 left-1 bg-red-900/90 text-red-300 text-xs px-1 rounded font-bold">🔐</div>
                        )}
                        ...
                        <div className="absolute inset-0 bg-black/70 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2 p-2">
                          ...
                          <button onClick={() => {
                            const idsToRemove = new Set(movie.streams.map(s => s.id));
                            saveStreams(streams.filter(x => !idsToRemove.has(x.id)));
                          }}
                      ...
                      <div className="p-2">
                        ...
                        {/* Source/group label */}
                        <div className="text-xs text-blue-400/70 truncate mt-0.5">{movie.streams[0]?.group || ''}</div>
                        {!meta?.tmdbId && (
                          <div className="mt-0.5 text-xs text-gray-700 italic">No TMDB</div>
                        )}
                      </div>
                    </div>
                  );
            ...
            <div className="bg-gray-900 border border-white/10 rounded-xl p-4 space-y-4">
              ...
              <div className="border-t border-white/10 pt-4">
                ...
                <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-3 mb-3">
                  ...
                  <p className="text-gray-400 text-xs">Set in Render / Koyeb / Railway. Backend auto-enriches movies on sync.</p>
                </div>

                <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-3">
                  <p className="text-amber-300 text-xs font-semibold mb-1">⚡ Browser key (for manual fetch)</p>
                  <p className="text-gray-400 text-xs mb-2">Used when you click "Fetch TMDB Metadata" in the Movies tab.</p>
                  <input type="password" placeholder="Paste your TMDB API key here…"
                  ...
                  {settings.tmdbApiKey && (
                    <p className="text-green-400 text-xs mt-1">✅ Key entered — click "Fetch TMDB Metadata" in Movies tab</p>
                  )}
                </div>
              </div>

              {[
                { key: 'autoFetchMetadata', label: 'Auto-fetch TMDB on source add', desc: 'Fetch metadata automatically when adding new sources' },
                { key: 'removeDuplicates',  label: 'Remove duplicate movies',       desc: 'Same title+year+url counted only once' },
                { key: 'combineQualities',  label: 'Combine quality streams',       desc: '720p/1080p of same movie shown as one entry with quality picker' },
              ].map(({ key, label, desc }) => (
          ...
          <div className="p-6 space-y-4 max-w-3xl">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                ...
                { label: 'Unique Movies',  value: combineByQuality(enabledStreams).length,  color: 'text-blue-400'   },
            ...
            <div className="bg-gray-900 border border-white/10 rounded-xl p-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-3">
                <div className="bg-gray-800 rounded-lg p-3">
                  <div className="text-yellow-400 text-xs font-bold mb-1">🔑 How catalogs work</div>
                  <div className="text-gray-400 text-xs space-y-1">
                    <div>• Each source = 1 separate catalog in Stremio</div>
                    <div>• Source name = catalog/group name</div>
                    <div>• Same movie in 2 sources → quality picker</div>
                    <div>• Global filters: Genre, Search, Rating, Year</div>
                  </div>
                </div>
                <div className="bg-gray-800 rounded-lg p-3">
                  <div className="text-blue-400 text-xs font-bold mb-1">🔍 Available Filters in Stremio</div>
                  <div className="text-gray-400 text-xs space-y-1">
                    <div>• 🔍 Search by title</div>
                    <div>• 📂 Genre filter</div>
                    <div>• 📅 Year filter (via genre)</div>
                    <div>• ⭐ Top Rated catalog (≥7.0)</div>
                    <div>• 🎬 HD catalog (720p/1080p)</div>
                  </div>
                </div>
              </div>

              {/* Per-source catalog preview */}
              {sources.length > 0 && (
                <div>
                  <p className="text-gray-400 text-xs mb-2">Your catalogs in Stremio:</p>
                  <div className="flex flex-wrap gap-2">
                    {['All Movies', '🎬 HD Movies', '⭐ Top Rated', '📅 By Year'].map(c => (
                      <span key={c} className="bg-orange-900/30 border border-orange-700/40 text-orange-300 text-xs px-2 py-1 rounded">{c}</span>
                    ))}
                    {sources.map(s => (
                      <span key={s.id} className="bg-blue-900/30 border border-blue-700/40 text-blue-300 text-xs px-2 py-1 rounded">📂 {s.name}</span>
                    ))}
                  </div>
                </div>
              )}
            ...
            <div className="flex flex-wrap gap-3">
              <button onClick={handleMovieSync}
                ...
                <span className={syncing ? 'animate-spin inline-block' : ''}>🔄</span>
                {syncing ? 'Syncing…' : `Sync ${enabledStreams.length} Movies to Backend`}
              </button>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900 border border-white/20 rounded-2xl p-6 w-full max-w-md space-y-4">
            ...
            {[
              ...
              { field: 'quality', label: 'Quality (1080p / 720p / 480p)' },
              { field: 'logo',    label: 'Poster / Logo URL' },
              { field: 'group',   label: 'Group / Source Name' },
            ].map(({ field, label }) => (
              <div key={field}>
